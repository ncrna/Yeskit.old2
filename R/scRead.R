...
scRead <- function(sample_name = NULL, data_dir = NULL, gene_column = 2, 
  ...
  if (!is.null(meta_features)) {
    ...
    object <- Seurat::CreateSeuratObject(counts = object, min.cells = min_cells,
      min.features = min_features, project = project_name)
  }
  ...
  if (!is.null(meta_file)) {
    if (file.exists(meta_file)) {
      metadata <- read.table(meta_file, header = TRUE, row.names = 1, 
  ...
  object <- subset(object, subset = nFeature_RNA > max(min_features, 
    quantile(object$nFeature_RNA, 0.01)) & 
    nFeature_RNA < quantile(object$nFeature_RNA, 0.99) & percent.mito <=
    min(percent_mito, quantile(object$percent.mito, 0.99)) & 
    nCount_RNA > max(min_rnas, quantile(object$nCount_RNA, 0.01)) & 
